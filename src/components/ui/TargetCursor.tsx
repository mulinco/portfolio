...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  useEffect(() => {
    ...
    const enterHandler = (e: MouseEvent) => {
      ...
      const leaveHandler = () => {
        ...
        Array.from(cornersRef.current!).forEach((corner, i) => {
          gsap.to(corner, { x: pos[i].x, y: pos[i].y, duration: 0.3, ease: 'power3.out' });
        });
  ...
  return (
    <div ref={cursorRef} className="fixed top-0 left-0 w-0 h-0 pointer-events-none z-[9999]">
      <div ref={dotRef} className="absolute top-1/2 left-1/2 w-1 h-1 rounded-full -translate-x-1/2 -translate-y-1/2 transition-colors duration-500" style={{ backgroundColor: cursorColor }} />
      {[
