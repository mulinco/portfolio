  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div
          className={`absolute top-[-5%] left-[-5%] w-[50%] h-[50%] rounded-full blur-[100px] opacity-30 ${currentIsKawaii ? "bg-pink-300" : "bg-red-900"}`}
        />
        <div
          className={`absolute bottom-[-5%] right-[-5%] w-[50%] h-[50%] rounded-full blur-[100px] opacity-30 ${currentIsKawaii ? "bg-pink-300" : "bg-red-900"}`}
        />
      </div>
      ...
      >
        ...
        {!currentIsKawaii && (
          <div className="bg-[#520815]/30 border-b border-[#520815] p-3 flex items-center gap-2">
            ...
            <span className="font-mono text-xs text-[#D2042D] opacity-70">
              root@portfolio:~
            </span>
          </div>
        )}
        ...
        <div className={`${!currentIsKawaii ? "p-10 md:p-16" : ""} space-y-10`}>
          <div className="space-y-4">
            ...
            <h2
              className={`text-3xl md:text-5xl font-bold mb-4 ${currentIsKawaii ? "font-cute text-[#D86487] tracking-tighter" : "font-mono text-[#D2042D] tracking-wider uppercase"}`}
            >
              {currentIsKawaii ? "Oie! Bem-vinde! ✨" : ">> ACCESS_GRANTED"}
            </h2>

            <div
              className={`h-1.5 rounded-full w-24 ${currentIsKawaii ? "bg-[#EEAAC3] mx-auto" : "bg-[#D2042D]"}`}
            />
          </div>

          <p
            className={`text-lg md:text-xl leading-relaxed ${currentIsKawaii ? "font-sans text-[#76172C] font-bold" : "font-code text-gray-400"}`}
          >
            {currentIsKawaii
      ...
      {/* Footer */}
      <div
        className={`absolute bottom-10 flex items-center gap-2 text-sm font-code font-bold ${currentIsKawaii ? "text-[#D86487]" : "text-gray-500"}`}
      >
        <span className="w-2.5 h-2.5 rounded-full bg-current animate-pulse" />
        {currentIsKawaii
          ? "✿ Magical Girl Mode ON ✿"
          : "SYSTEM_STABLE // v1.0.5"}
      </div>
    </motion.div>
  );
};
