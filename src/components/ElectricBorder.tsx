...
const ElectricBorder: React.FC<ElectricBorderProps> = ({
  ...
  const getRoundedRectPoint = useCallback(
    (t: number, width: number, height: number, radius: number) => {
      ...
      if (dist <= (acc += sh))
        return { x: width, y: radius + (dist - (acc - sh)) };
      if (dist <= (acc += arc))
        return corner(
          width - radius,
          height - radius,
      ...
      if (dist <= (acc += sw))
        return { x: width - radius - (dist - (acc - sw)), y: height };
      if (dist <= (acc += arc))
        return corner(
          radius,
          height - radius,
      ...
      if (dist <= (acc += sh))
        return { x: 0, y: height - radius - (dist - (acc - sh)) };
      return corner(
        radius,
        radius,
        radius,
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none z-[2] transition-opacity duration-500 ${isHovered ? "opacity-100" : "opacity-0"}`}
      >
      ...
      <div
        className={`absolute inset-0 rounded-[inherit] pointer-events-none z-0 transition-opacity duration-500 ${isHovered ? "opacity-100" : "opacity-0"}`}
      >
        <div
