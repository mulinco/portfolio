...
const STYLES = {
  kawaii: {
    ...
    lineColor: "bg-[#EEAAC3]",
    iconContainer:
      "bg-white border-[#EEAAC3] text-[#D86487] shadow-[0_0_15px_rgba(216,100,135,0.2)]",
    ghostDate:
      "text-[#D86487]/80 font-cute text-8xl select-none drop-shadow-sm",
  ...
  goth: {
    ...
    lineColor: "bg-accent",
    iconContainer:
      "bg-bg-secondary border-accent text-accent shadow-[0_0_15px_rgba(210,4,45,0.3)]",
    ghostDate:
      "text-accent/70 font-display text-9xl italic tracking-tighter select-none drop-shadow-[0_4px_12px_rgba(0,0,0,0.8)]",
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span
            className={`inline-block mb-3 text-xs font-bold tracking-widest px-3 py-1 rounded-full border ${isKawaii ? "bg-pink-100 border-pink-200 text-pink-500" : "bg-accent/10 border-accent/20 text-accent"}`}
          >
            {year}
          </span>
          <h3
            className={`mb-2 uppercase ${theme.titleColor} ${theme.fontTitle}`}
          >
            {title}
          </h3>
          <p
            className={`text-sm leading-relaxed ${theme.bodyColor} ${theme.fontBody}`}
          >
            {description}
          </p>
        </div>
    ...
    <div className="relative flex flex-col md:flex-row items-center justify-center w-full mb-24 last:mb-0">
      ...
      <div className="hidden md:flex w-[45%] justify-end pr-16 items-center">
        {isEven ? (
          ...
          <span
            className={`select-none uppercase tracking-tighter ${theme.ghostDate}`}
          >
            {year.split(" ")[0]}
          </span>
        )}
...
export const LifeTimeline = ({
  ...
  const events = [
    {
      year: "2019 - 2020",
      title: "Letras (UFRJ)",
      description: "Busca por desafios científicos.",
      icon: <BookOpen size={20} />,
    },
    {
      year: "2020 - 2026",
      title: "Biologia e Genética",
      description: "Metodologia e pensamento analítico.",
      icon: <Dna size={20} />,
    },
    {
      year: "2024",
      title: "A Mutação (O 'Click')",
      description: "Descoberta da automação com Python.",
      icon: <Code size={20} />,
    },
    {
      year: "2025",
      title: "ADS & Full Stack",
      description: "Transição total para tecnologia.",
      icon: <GraduationCap size={20} />,
    },
    {
      year: "2026",
      title: "Futuro",
      description: "Engenharia de Software e Genética Digital.",
      icon: <Rocket size={20} />,
    },
  ];
  ...
  return (
    <section className="relative pt-32 pb-40 px-4 md:px-0 w-full overflow-hidden">
      ...
      >
        <h2
          className={`text-5xl md:text-7xl font-display tracking-tighter ${isKawaii ? "text-[#D86487] font-cute" : "text-accent font-bold"}`}
        >
          {isKawaii ? "Minha Doce Jornada" : "Evo_Logging"}
        </h2>
        <div
          className={`h-1 w-24 mx-auto ${isKawaii ? "bg-pink-300" : "bg-accent"}`}
        />
      </motion.div>

      <div ref={containerRef} className="max-w-6xl mx-auto relative">
        <div
          className={`absolute left-1/2 -translate-x-1/2 top-[32px] bottom-[32px] w-1 opacity-20 ${isKawaii ? "bg-pink-200" : "bg-accent/20"}`}
        />
