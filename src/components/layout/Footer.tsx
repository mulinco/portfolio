      ...
      <div className="container mx-auto max-w-6xl relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-16">
          ...
          <div className="space-y-6">
            <div>
              <h2
                className={`text-3xl font-bold font-display mb-2 ${isKawaii ? "text-[#D86487] font-cute" : "text-text-primary"}`}
              >
                mulinco
                <span className={isKawaii ? "text-pink-400" : "text-accent"}>
                  .dev
                </span>
              </h2>
              <p
                className={`font-sans text-sm leading-relaxed max-w-xs ${isKawaii ? "text-[#76172C]" : "text-text-secondary"}`}
              >
                De células a códigos. Construindo soluções digitais com a
                precisão de uma cientista e a criatividade de uma artista.
              </p>
            </div>
            ...
            <div
              className={`inline-flex items-center gap-2 px-3 py-1 rounded-full border ${
                isKawaii
                  ? "bg-white/40 border-pink-200"
                  : "bg-black/40 border-accent/20"
              }`}
            >
              <span className="relative flex h-2 w-2">
                <span
                  className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${isKawaii ? "bg-pink-400" : "bg-accent"}`}
                ></span>
                <span
                  className={`relative inline-flex rounded-full h-2 w-2 ${isKawaii ? "bg-pink-500" : "bg-accent"}`}
                ></span>
              </span>
              <span
                className={`text-xs font-bold tracking-wide uppercase ${isKawaii ? "text-pink-600" : "text-text-primary"}`}
              >
          ...
          <div className="md:pl-10">
            <h3
              className={`text-lg font-bold font-display mb-6 ${isKawaii ? "text-[#D86487]" : "text-text-primary"}`}
            >
              Explorar
            </h3>
            <ul
              className={`space-y-3 font-code text-sm ${isKawaii ? "text-[#76172C]" : "text-text-secondary"}`}
            >
              <li>
                <a
                  href="#projetos"
                  className="cursor-target hover:text-accent hover:pl-2 transition-all block"
                >
                  Projetos
                </a>
              </li>
          ...
          <div>
            ...
            <div className="space-y-4 mb-6">
                  ...
                  <span
                    className={`text-sm font-sans transition-colors ${
                      copied
                        ...
                        : isKawaii
                          ? "text-[#76172C] group-hover:text-pink-600"
              ...
              <div className="cursor-target flex items-center gap-3">
                <div
                  className={`p-2 rounded border ${isKawaii ? "bg-white/40 border-pink-200 text-pink-500" : "bg-black/40 border-accent/20 text-text-secondary"}`}
                >
                  <MapPin size={18} />
                </div>
                <span
                  className={`text-sm font-sans ${isKawaii ? "text-[#76172C]" : "text-text-secondary"}`}
                >
        ...
        {/* --- DIVISÓRIA --- */}
        <div
          className={`h-px w-full mb-8 bg-gradient-to-r from-transparent via-accent/30 to-transparent ${isKawaii ? "via-pink-400/30" : ""}`}
        ></div>
        ...
        <div className="cursor-target flex flex-col md:flex-row justify-between items-center gap-4 text-xs md:text-sm font-code opacity-80">
          <div className="flex items-center gap-1">
            ...
            <span className="text-accent inline-block mx-1">
              ...
              <Heart
                size={14}
                className={isKawaii ? "inline animate-pulse" : "hidden"}
                fill="currentColor"
              />
            </span>
