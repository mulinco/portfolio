...
import { X } from 'lucide-react'; // Certifique-se de ter o lucide-react ou use outro ícone/texto "X"

// 1. Definimos a tipagem dos dados para o TypeScript ficar feliz
interface Pet {
  name: string;
  title: string;
  image: string;
...
export const PetSlider = () => {
  // 2. Estado para controlar qual pet está selecionado (null = nenhum)
  ...
  // Ajuste fino para caso o aniversário ainda não tenha chegado neste ano
  ...
  // Se tiver 1 ano exato
  if (years === 1) return "1 ano";
  // Caso contrário, mostra anos
  return `${years} anos`;
};
      ...
      <div className="flex gap-6 overflow-x-auto snap-x snap-mandatory scrollbar-hide p-8 max-w-4xl mx-auto">
        {pets.map((pet, index) => (
          <div 
            ...
            onClick={() => setSelectedPet(pet)} // Ao clicar, define este pet como selecionado
          >
            <div className="bg-bg-secondary border-2 border-accent/20 rounded-3xl p-6 transition-all hover:border-accent hover:scale-[1.02] group kawaii:bg-white kawaii:shadow-xl h-full">
      ...
      {selectedPet && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          ...
          <div className="relative bg-bg-secondary w-full max-w-lg rounded-3xl border-2 border-accent p-6 md:p-8 shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            >
              {/* Se não tiver o lucide-react instalado, troque <X /> por <span>X</span> */}
              <X size={32} />
            </button>
            ...
            <div className="flex flex-col items-center">
              <img 
                ...
                className="w-32 h-32 rounded-full border-4 border-accent object-cover mb-4"
              />
